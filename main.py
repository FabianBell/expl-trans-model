...
async def translate(
    ...
    ignore = [i for i, sentence in enumerate(
        sentences) if not model.check_length(sentence)]
    for i in reversed(ignore):
        sentences.pop(i)
    results = [result for batch in batch_loader(
        sentences) for result in model.translate(batch)]
    for i in ignore:
...
class Entry(BaseModel):
    ...
    positions: List[List[Tuple[int, int]]] = Field(..., example=[
                                                   [(10, 15), (16, 20)]])
...
class EntryIter:
    ...
    def __next__(self):
        if self.pos >= len(self.entry.sentences):
            raise StopIteration
        out = (self.entry.sentences[self.pos],
               self.entry.trans_sentences[self.pos],
               self.entry.positions[self.pos])
        self.pos += 1
        return out
...
async def backward(entry: Entry):
    if len(
        entry.sentences) == 0 or len(
        entry.trans_sentences) == 0 or len(
            entry.positions) == 0:
        raise HTTPException(status_code=400, detail='No sentences given.')
    if not len(
        entry.sentences) == len(
        entry.trans_sentences) == len(
            entry.positions):
        raise HTTPException(
            status_code=400,
            detail='All parameters must have the same batch dimension.')
    if any([any([l < 0 or r <= 0 or l == r or l >= len(entry.trans_sentences[i]) or r > len(
            entry.trans_sentences[i]) or l > r for l, r in pos]) for i, pos in enumerate(entry.positions)]):
        raise HTTPException(
            status_code=400,
            detail='Invalid or out of range character positions')
    ignore = []
    for i, (sentence, trans_sentence, _) in enumerate(entry):
        if not (model.check_length(sentence)
                and model.check_length(trans_sentence)):
            ignore.append(i)
    for i in reversed(ignore):
    ...
    out = [pred for batch in batch_loader(entry)
           for pred in model.backward(*zip(*batch))]
    out = [list(set([elem for mapping in row for elem in mapping]))
           for row in out]
    for i in ignore:
        out.insert(i, None)
    return out
